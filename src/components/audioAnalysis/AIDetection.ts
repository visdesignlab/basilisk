...
import { EventType, MouseEventType, StoredAnswer } from "../../store/types";
...
function mouseMove(answer: StoredAnswer): FlaggedEvent[] {
    ...
    const calcMouseMove = (e1: MouseEventType, e2: MouseEventType) => {
        const deltaT = e2[0] - e1[0];
        const deltaX = Math.abs([2][0] - e1[2][0]);
        const deltaY = Math.abs([2][1] - e1[2][1]);
        const distance = Math.sqrt(deltaX*deltaX + deltaY*deltaY);
        return {deltaT, deltaX, deltaY, distance}
    }
    ...
    for (const mouseEvent of mouseEvents) {
        ...
        if (mouseEvent[1] === 'mouseup' && mouseDown) {
        ...
        if(i > 10) {
            ...
            for(let j = 1; j < 10 ; j++) {
                if(timeBetween === -1) {
                    ...
                    if(typingEvents[i - j][0] - typingEvents[i - j + 1][0] !== timeBetween) {
                        consistent = false;
                        break;
                    }
                }
